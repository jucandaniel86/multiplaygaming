...
onMounted(async () => {
  loading.value = true;
  await getOptions(['game_assets_name', 'game_assets_link']);
  loading.value = false;
});

const openDownloads = () => {
  if (!options.value.game_assets_link) return;

  window.open(options.value.game_assets_link, '_blank');
}
